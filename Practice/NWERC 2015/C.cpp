...
int main() {
    ...
    int n, m; cin >> n >> m;
    vector<P> W(n);
    for (auto &p : W) cin >> p.x >> p.y;
    vector<S> seg;
    for (int i = 0 ; i < m ; i++) {
        int idx; P p; cin >> idx >> p.x >> p.y; idx--;
        seg.push_back(S{W[idx], p});
    }
    ...
    for (int i = 0 ; i < m ; i++) {
        for (int j = 0 ; j < i ; j++) {
            if (!intersect(seg[i], seg[j]))
                continue;
            P p = intersection(seg[i], seg[j]);
            if (p == seg[i].p1 || p == seg[j].p1)
                continue;
    ...
    for (int i = 0 ; i < m ; i++) {
        if (c[i]) continue;
        c[i] = 1; queue<int> q; q.push(i); while (q.size()) {
            int p = q.front(); q.pop();
            for (auto &v : G[p]) {
                if (c[v] == 0) {
                    c[v] = c[p] * -1;
